...
//Verificar la fecha 
function VerificarFecha(fechaRecibida){
    var fechaRecibida = new Date(fechaRecibida);
    //Milesgundos a Fechas Normales 
    fechaRecibida = fechaRecibida.toLocaleDateString();
    var fechaRecibida2 = fechaRecibida.split("/");
    var fechaActual = new Date(Date.now()).toLocaleDateString().split("/");
    
    if(fechaRecibida2[0] = fechaActual[0]){
        if(fechaRecibida2[0] < fechaActual[0]){
            return 0;
        }
    }else{
        if(fechaRecibida2[1] < fechaActual[1]){
            return 0;
        }
    }
    return 1;
...
function getAge(birthday) {
    var ageDifMs = Date.now() - birthday;
    var ageDate = new Date(ageDifMs); // miliseconds from epoch
    return Math.abs(ageDate.getUTCFullYear() - 1970) + VerificarFecha(birthday);
}
